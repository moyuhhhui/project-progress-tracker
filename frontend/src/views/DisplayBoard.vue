    ...
    <div v-else class="board-planning">
      <section class="planning-panel timeline-panel" aria-label="关键时间轴">
        ...
        <div v-if="timelineOverview.groups.length" class="timeline">
          <div v-for="group in timelineOverview.groups" :key="group.date" class="timeline-group" :class="{ 'is-overdue': group.items.some(item => timelineStatusLabel(item) === '逾期'), 'is-today': group.date === currentDate, 'is-future': group.date > currentDate }">
            ...
            <div class="timeline-items">
              <article v-for="item in group.items" :key="item.id" class="timeline-item" :class="[`status-${arrangementPresentation(item).tone}`, `project-tone-${projectColorTone(item.projectId)}`, { overdue: timelineStatusLabel(item) === '逾期' }]">
                <span class="timeline-project-block"><span class="item-project">{{ item.code }} · {{ item.projectName }}</span><span v-if="item.owners.some(person => person.name !== '待明确')" class="timeline-owners"><span v-for="person in item.owners" :key="`${person.role}-${person.name}`"><b :class="`role-${ownerRoleTone(person.role)}`">{{ ownerRoleLabel(person.role, person.primary) }}</b>{{ person.name }}</span></span></span><strong>{{ item.target }}</strong>
                <div class="item-meta"><span v-if="item.owner !== '待明确'">{{ item.owner }}</span><b>{{ timelineStatusLabel(item) }}</b></div>
              </article>
            </div>
          </div>
        </div>
