...
import { api } from '../api'
import { arrangementPresentation, dateTime, errorText, isSnapshotStale, planningOverview, projectColorTone, today } from '../domain'
...
const timelineOverview = computed(() => planningOverview(projects.value, currentDate.value))
const calendarOverview = computed(() => planningOverview(projects.value, calendarStart.value || currentDate.value))
  ...
  <main class="display-board">
    ...
    <div v-else class="board-planning">
      <section class="planning-panel timeline-panel" aria-label="关键时间轴">
        <div class="panel-heading"><div><p class="board-kicker">TIMELINE</p><h2>关键时间轴</h2></div><span>已定日期的项目事项</span></div>
      ...
      <section class="planning-panel calendar-panel" aria-label="安排日历">
        <div class="panel-heading"><div><p class="board-kicker">CALENDAR</p><h2>安排日历</h2></div><span>{{ calendarStart || currentDate }} — {{ calendarOverview.endDate }}</span></div>
        <div class="calendar-toolbar"><p class="panel-hint">{{ calendarOverview.calendarActiveCount }} 项进行中 · 跨天事项每天展示</p><el-date-picker v-model="calendarStart" type="date" value-format="YYYY-MM-DD" placeholder="选择起始日期" aria-label="日历起始日期" clearable /></div>
