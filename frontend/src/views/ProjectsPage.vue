...
const filtered = computed(() => props.projects.filter(p =>
  ...
  (!stateFilter.value || p.status === stateFilter.value) && (!riskOnly.value || !!p.flags?.length)))
...
<template>
  <div class="page-heading progress-heading"><div><h1>项目进度</h1><p class="muted">把每一步推进，都变成看得见的成就。</p></div><div class="heading-actions"><p class="progress-cheer">✦ 今天也在稳稳向前 · {{ mood.total }} 个项目</p><el-button @click="edit('create_meeting')">＋ 记录会议</el-button><el-button type="primary" @click="edit('create_project')">＋ 新建项目</el-button></div></div>
  ...
  <section class="progress-sheet" aria-label="全部项目进度表">
    ...
    <el-table v-loading="loading" :data="visibleProjects" row-key="id" class="progress-table" :row-class-name="projectRowClass" :empty-text="projects.length ? '没有符合筛选条件的项目' : '暂无项目，点击右上角新建项目'" @row-click="(project: Project) => selectedId = project.id">
      ...
      <el-table-column label="操作" width="60" fixed="right"><template #default="{ row }"><el-button link type="primary" @click.stop="selectedId = row.id">查看</el-button></template></el-table-column>
    </el-table>
  ...
  <ActionEditor v-if="editor" :key="`${editor.intent}-${editor.project?.id}-${editor.node?.id}`" v-bind="editor" :actor="actor" :users="users" @close="editor = null" @saved="$emit('saved')" />
</template>
