...
import type { Actor, Draft, Intent, Milestone, Project, User } from '../types'
...
<template>
  ...
  <ActionEditor v-if="editor" :key="`${editor.intent}-${editor.project?.id}-${editor.node?.id}`" v-bind="editor" :actor="actor" :users="users" @close="editor = null" @draft="$emit('draft', $event)" />
</template>
