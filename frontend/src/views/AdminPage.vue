...
<template>
  ...
  <section class="panel section-gap"><h2>集成状态</h2><div class="integration-grid"><div><span class="muted">自然语言模型</span><p>{{ status?.ai_configured ? `已配置：${status.ai_model}` : '尚未配置' }}</p></div><div><span class="muted">企微群消息接收</span><p>{{ status?.message || '正在读取连接状态' }}</p></div><div><span class="muted">企微主动发送开关</span><p>{{ status?.wecom_send_enabled ? '已开启，实际送达请查看记录' : '未开启' }}</p></div></div><p class="muted">在群内直接 @机器人提交文字，无需绑定；信息齐全自动保存，缺项保留草稿。</p></section>
