...
| `WECOM_GROUP_WEBHOOK_URL` | 可选；配置后主动提醒改用企业微信群机器人 Webhook，统一发送到该群；不要提交真实 Webhook |
...
Web 服务不会自动启动 worker。管理员页面“扫描提醒”只更新队列，不发送外部消息。主动发送需要显式开启发送开关；配置 `WECOM_GROUP_WEBHOOK_URL` 后提醒会合并发送到该群，否则按负责人 UserID 通过企业微信应用发送。只有具备真实权限验证和授权后再开启。
...
- 同一节点每天最多一条成功提醒，多种原因合并；个人模式下同一负责人的多个节点按消息大小合并，群模式下所有节点按消息大小合并。未启用人员进入异常记录；群模式不要求负责人绑定 UserID。
